	...
	function Reg(){
		...
		if (login.length < 6) {
			...
			for (let i = 0; i < login.length; i++)
				if (login[i] == ' ') {
					document.getElementById('descr1').innerHTML = 'Логин не должен содержать пробелов';
					stopReg = true;
					break;
				}
		}
		...
		for (let i = 0; i < pass.length; i++)
			if (pass[i] == ' ') {
					document.getElementById('descr2').innerHTML = 'Пароль не должен содержать пробелов';
					stopReg = true;
					break;
				}
		...
		for (let i = 0; i < pass.length; i++)
			if (pass[i] == '$' || pass[i] == '?' || pass[i] == '#' || pass[i] == '@' || pass[i] == '+') {
					document.getElementById('descr2').innerHTML = 'Пароль не должен содержать спецсимволов';
					stopReg = true;
					break;
				}
...
<body>
	<div id="window-box">
		...
		Повторите пароль:<br>
		<input type="password" id="password2" placeholder="Подтвердите пароль"><br>
